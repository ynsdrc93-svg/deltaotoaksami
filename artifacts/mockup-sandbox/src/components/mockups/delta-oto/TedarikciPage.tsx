        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 h-24 flex items-center justify-between">
          <div className="flex items-center gap-10">
            ...
            <nav className="hidden lg:flex items-center gap-6 text-[13.5px] font-medium text-slate-600">
              {NAV.map(l => (
                <a key={l} href="#" className={`hover:text-[#1B3A8F] transition-colors whitespace-nowrap ${l === "Tedarikçiler" ? "text-[#1B3A8F] font-semibold border-b-2 border-[#1B3A8F] pb-0.5" : ""}`}>{l}</a>
              ))}
            </nav>
          </div>
      ...
      <section className="relative min-h-[560px] flex items-center text-white overflow-hidden bg-[#0e1016]">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10 py-28">
          ...
          <h1 className="text-5xl md:text-6xl lg:text-[72px] font-black leading-[1.05] tracking-[-0.02em] mb-6">
            ...
            <span className="text-[#7d9bea]">RAKABET ÜSTÜNLÜĞÜ</span>
          </h1>
          <p className="text-[17px] text-gray-300 leading-[1.8] max-w-2xl mb-10 font-light">
            Binek ve hafif ticari araç kategorilerinde 250'den fazla marka ve 50.000'i aşkın SKU; tek tedarikçi ilişkisiyle eksiksiz karşılanır. Geniş portföy, anlık stok erişimi, güvenceli teslimat.
          </p>
            ...
            {CATEGORIES.map((cat) => (
              <div key={cat.name} className="border border-slate-200 rounded-xl p-7 hover:border-[#1B3A8F]/40 hover:shadow-lg transition-all group cursor-pointer">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="shrink-0 ml-3 text-[11px] font-bold text-[#1B3A8F] bg-[#1B3A8F]/8 border border-[#1B3A8F]/15 px-2.5 py-1 rounded">{cat.count}</span>
                </div>
      ...
      <section className="bg-[#1B3A8F] py-24 text-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {ADVANTAGES.map((a) => (
              <div key={a.title} className="bg-white/8 backdrop-blur border border-white/12 rounded-xl p-8 hover:bg-white/14 transition-colors">
